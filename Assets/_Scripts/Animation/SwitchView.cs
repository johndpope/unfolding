...
public class SwitchView : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {

	public UnityEvent ModelViewSelected;
	public UnityEvent DrawingViewSelected;
	public RectTransform ReferencePageRectTransform;
	public Image ViewSwitchRectBG;
	public CanvasGroup CurrentViewIndicatorCanvasGroup;
	public Text UpcomingViewName;
	public int CurrentPageNumber;
	public GraphicRaycaster GraphicRaycaster;
	public RectTransform[] PaginationDots;

	private int previousPageNumber = 0;
	private float maxNormalisedPosition = 2.5f;
	private float initialReferencePositionX;
	private float previousReferencePositionX;
	private bool modelViewIsOn = false;
	private bool drawingViewIsOn = false;
	...
	void Start() {
		...
		//
		initialReferencePositionX = ReferencePageRectTransform.position.x;
		PaginationDots[0].localScale = new Vector3(1.25f, 1.25f, 1.25f);
		SwitchedToModelView();
	}
	...
	void Update() {
		...
		if (_lerp) {
			...
			CurrentPageNumber = GetNearestPage();

			SwitchedToModelView();
			SwitchedToDrawingView();

			if(previousPageNumber != CurrentPageNumber)
			{
				PaginationDots[CurrentPageNumber].localScale = new Vector3(1.25f, 1.25f, 1.25f);
				PaginationDots[previousPageNumber].localScale = new Vector3(1f, 1f, 1f);
			}
			...
			float normalisedSnapPosition = Mathf.Clamp(Mathf.Abs(ReferencePageRectTransform.position.x), 0f, initialReferencePositionX).Map(0f, initialReferencePositionX, 5f, 0f);
			CurrentViewIndicatorCanvasGroup.alpha = normalisedSnapPosition;
			ViewSwitchRectBG.color = new Color(0.12f, 0.12f, 0.12f, normalisedSnapPosition);

			if(normalisedSnapPosition > 1f)
			{
			...
			if(normalisedSnapPosition < 0.01f)
			{
				GraphicRaycaster.enabled = true;
			}
		previousReferencePositionX = ReferencePageRectTransform.position.x;
	}

	void SwitchedToModelView()
	{
		if(CurrentPageNumber == 0)
		{
			ModelViewSelected.Invoke();
			if(!_lerp)
			{
				UpcomingViewName.text = "ARCHITECTURE DRAWING";
				GraphicRaycaster.enabled = true;
			}
		}
	}

	void SwitchedToDrawingView()
	{
		if(CurrentPageNumber == 1)
		{
			DrawingViewSelected.Invoke();
			if(!_lerp)
			{	
				UpcomingViewName.text = "3D MODEL";
				GraphicRaycaster.enabled = true;
			}
		}
	}
